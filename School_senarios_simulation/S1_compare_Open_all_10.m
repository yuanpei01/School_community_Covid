...
%% after school open
% School close
par1_Open_all_10=[par10(1:6) sim_all_10];
...
for i=T1:Num_fit
    if open_count<=5
       ...
       par2_s1=[par2(1:20);1];
       [t1,y1] = ode45(@NCoVGTA_Household_openSchool_sim,[t1(end):1/24:t1(end)+6/24],y02,[],par1_Open_all_10,par2_s1,AC,CC); 
       y1_temp=y1;
       y3=[y3(1:end-1,:);y1];
       y03=[y1_temp(end,1:10)';sc;y1_temp(end,16:352)'];
       par2_s2=[par2(1:20);0];
       [t1,y1] = ode45(@NCoVGTA_Household_openSchool_sim,[t1(end):1/24:t1(end)+18/24],y03,[],par1_Open_all_10,par2_s2,AC,CC); 
       y3_temp=y1;
       y3=[y3(1:end-1,:);y1];
    else
        par2_s2=[par2(1:20);0];
        y03=[y3_temp(end,1:10)';sc;y3_temp(end,16:352)'];
        [t1,y1] = ode45(@NCoVGTA_Household_openSchool_sim,[t1(end):1/24:t1(end)+1],y03,[],par1_Open_all_10,par2_s2,AC,CC); 
        y3_temp=y1;
        y3=[y3(1:end-1,:);y1];
    end
 if open_count==7
...
y_Open_all_10=[y1_copy(1:T1-1,:);y3(T1+23:24:end,:)];
y2_Open_all_10=[y1_copy(1:T1-1,:);y3(T1+4:24:end,:)]; % for school community

% CumD
CumD_Open_all_10=y_Open_all_10(:,21);
% CumI
CumIa_Open_all_10=y_Open_all_10(:,22);
CumIc_Open_all_10=y_Open_all_10(:,23);

%NEW infection (A+I) of time t
% Ia_newt=1/tau1*(Ega)
Ia_newt_total_Open_all_10=1/tau1.*(y_Open_all_10(:,2));
Ic_newt_total_Open_all_10=1/tau1.*(y_Open_all_10(:,7));

% New CASE (I) of time t
%I2_newt=1/tau2*(I1)
% Ia2_newt=1/tau2.*(y_total(:,4));
% Ic2_newt=1/tau2.*(y_total(:,9));
Ia2_newt_Open_all_10=[Inew_ga_0;diff(CumIa_Open_all_10)];
Ic2_newt_Open_all_10=[Inew_gc_0;diff(CumIc_Open_all_10)];
D_newt_Open_all_10=[Dnew_0;diff(CumD_Open_all_10)];

% new case in School
Isc_Open_all_10=y2_Open_all_10(:,15);

save S1_compare_Open_all_10
...
subplot(2,2,1)
plot(1:N_plot,Ia_newt_total_Open_all_10,'-','linewidth',2) 
...
set(gca, 'Box', 'off', 'TickDir', 'out', 'TickLength', [.02 .02], ...
    ...
    'LineWidth', 1)
subplot(2,2,2)
plot(1:N_plot,Ic_newt_total_Open_all_10,'-','linewidth',2) 
legend('school open, with community partily open')
...
set(gca, 'Box', 'off', 'TickDir', 'out', 'TickLength', [.02 .02], ...
    ...
    'LineWidth', 1)
subplot(2,2,3)
plot(1:N_plot,D_newt_Open_all_10,'-','linewidth',2) 
legend('school open, with community partily open')
...
set(gca, 'Box', 'off', 'TickDir', 'out', 'TickLength', [.02 .02], ...
    ...
    'XColor', [.3 .3 .3], 'YColor', [.3 .3 .3], ...
    'LineWidth', 1)
print('-f2','-djpeg','-r600','./school_plot_revision/S1_compare_Open_all_10.jpeg')
saveas(gcf,'./school_plot_revision/S1_compare_Open_all_10.fig')
toc
