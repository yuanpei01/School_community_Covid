...
Tor_R0_ga= readmatrix('Rt_QC2_withSchoolClose_ga');
Tor_R0_gc= readmatrix('Rt_QC2_withSchoolClose_gc');
...
colormap(parula)
% colorbar
text(7.82-0.21,0.0085-0.0006,'*','color','r','FontSize',40)
...
xlabel('Contact rate(adult)')
ylabel('Home transmission risk')
title('B  R_t of C&Y population on Oct 5,2020')
set(get(gca,'XLabel'),'FontSize',14);%?????8 point??5?
set(get(gca,'YLabel'),'FontSize',14);
set(get(gca,'Children'),'linewidth',2.0);  %?????'linewidth'??????'MarkerSize'
%set(get(gca,'Children'),'MarkerSize',10);
...
Tor_R0_ga= readmatrix('Rt_BC2_withSchoolClose_ga');
Tor_R0_gc= readmatrix('Rt_BC2_withSchoolClose_gc');
...
colormap(parula)
% colorbar
text(7.82-0.21,0.041-0.0012,'*','color','r','FontSize',40)
...
set(gca, 'Box', 'off', 'TickDir', 'out', 'TickLength', [.02 .02], ...
    ...
    'XColor', [.3 .3 .3], 'YColor', [.3 .3 .3], ...
    'LineWidth', 1)
% print('-f4','-djpeg','-r600','./School_plot_revision/School_FigA3.jpeg')
% saveas(gcf,'./School_plot_revision/School_FigA3.fig')
