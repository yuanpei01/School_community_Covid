...
%% after school open
% School close
par1_schoolClose_Phase2=[par10(1:6) sim_phase2];
par2(end)=0;
[t_schoolClose_Phase2,y_schoolClose_Phase22] = ode45(@NCoVGTA_Household_openSchool_sim,[39:Num_fit],y01,[],par1_schoolClose_Phase2,par2,AC,CC);
y_schoolClose_Phase2=[y_total(1:end-1,:);y_schoolClose_Phase22];

% CumD
CumD_schoolClose_Phase2=y_schoolClose_Phase2(:,21);
% CumI
CumIa_schoolClose_Phase2=y_schoolClose_Phase2(:,22);
CumIc_schoolClose_Phase2=y_schoolClose_Phase2(:,23);

%NEW infection (A+I) of time t
% Ia_newt=1/tau1*(Ega)
Ia_newt_total_schoolClose_Phase2=1/tau1.*(y_schoolClose_Phase2(:,2));
Ic_newt_total_schoolClose_Phase2=1/tau1.*(y_schoolClose_Phase2(:,7));

% New CASE (I) of time t
%I2_newt=1/tau2*(I1)
% Ia2_newt=1/tau2.*(y_total(:,4));
% Ic2_newt=1/tau2.*(y_total(:,9));
Ia2_newt_schoolClose_Phase2=[Inew_ga_0;diff(CumIa_schoolClose_Phase2)];
Ic2_newt_schoolClose_Phase2=[Inew_gc_0;diff(CumIc_schoolClose_Phase2)];
D_newt_schoolClose_Phase2=[Dnew_0;diff(CumD_schoolClose_Phase2)];

save S1_compare_schoolClose_Phase2
...
subplot(2,2,1)
plot(1:N_plot,Ia_newt_total_schoolClose_Phase2,'-','linewidth',2) 
...
set(gca, 'Box', 'off', 'TickDir', 'out', 'TickLength', [.02 .02], ...
    ...
    'LineWidth', 1)
subplot(2,2,2)
plot(1:N_plot,Ic_newt_total_schoolClose_Phase2,'-','linewidth',2) 
legend('school open, with community partily open')
...
set(gca, 'Box', 'off', 'TickDir', 'out', 'TickLength', [.02 .02], ...
    ...
    'LineWidth', 1)
subplot(2,2,3)
plot(1:N_plot,D_newt_schoolClose_Phase2,'-','linewidth',2) 
legend('school open, with community partily open')
...
set(gca, 'Box', 'off', 'TickDir', 'out', 'TickLength', [.02 .02], ...
    ...
    'XColor', [.3 .3 .3], 'YColor', [.3 .3 .3], ...
    'LineWidth', 1)
print('-f2','-djpeg','-r600','./school_plot_revision/S1_compare_schoolClose_Phase2.jpeg')
saveas(gcf,'./school_plot_revision/S1_compare_schoolClose_Phase2.fig')
toc
